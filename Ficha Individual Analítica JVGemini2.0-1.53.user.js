// ==UserScript==
// @name         Ficha Individual Analítica JVGemini2.0
// @namespace    http://tampermonkey.net/
// @version      1.53
// @description  Ferramentas para analisar a ficha individual do GPE/Sigeduca
// @author       Lucas S Monteiro & João Vitor C F
...
`;
    ...
    tabelaHTML += '<h3 style="text-align:center;"> Mapa para Aprovação (P/Aprov) - '+dataArray.dadosTurma.turma+' '+dataArray.dadosTurma.turno+'</h3>';
    tabelaHTML += '<table border="1"><thead><tr>';
    tabelaHTML += '<th>Codigo</th><th>nome</th><th>Situa.</th>';
    ...
    // VARIÁVEL DE PARAMETRO: ALTERE ESTE VALOR PARA MUDAR A REFERÊNCIA
    var notaReferencia = 7.0;

    Object.keys(dataArray.alunos).forEach(function eachKey(cod) {
        var mediaano = new Array(dataArray.materias.length).fill(0);

        // Calcula somas
        for (var i = 0; i < dataArray.materias.length; i++) {
             if (dataArray.alunos[cod].notas[dataArray.materias[i]] != undefined) {
                 for(var k=1; k<=4; k++){
                     var val = dataArray.alunos[cod].notas[dataArray.materias[i]]['N' + k];
                     if (val != undefined && !isNaN(val)) {
                         mediaano[i] += Number(val);
                     }
                 }
             }
        }
        ...
        for (var u = 0; u < dataArray.materias.length; u++) {
            ...
            var corFundo = 'white'; // Padrão sem cor (branco)

            if (mediaApr == 24){
                tabelaHTML += '<td style="text-align:center;background-color: white ">-</td>';
            } else if (mediaApr <= 0){
                tabelaHTML += '<td style="text-align:center;background-color: #b3f538 ">AP</td>'; // Verde claro para aprovado
            } else {
                // Se a nota que falta for maior que o parametro (6.0), pinta de vermelho claro
                if (parseFloat(mediaApr) > notaReferencia) {
                    corFundo = '#ffcccc';
                }
                tabelaHTML += '<td style="text-align:center;background-color: ' + corFundo + ' ">' + mediaApr + '</td>';
            }
        }
        tabelaHTML += '</tr>';
    });
...
(function() {
    ...
    // Mapa Para Aprovação (NOVO) ----
    subButton = document.createElement('div');
    subButton.className = "menu-item";
    subButton.textContent = 'Mapa de notas para aprovação';
    subButton.addEventListener('click', function() {
