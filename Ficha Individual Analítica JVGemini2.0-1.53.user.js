// ==UserScript==
// @name         Ficha Individual Analítica JVGemini5.0 (recomposição completa + conferência notas excel)
...
function MapaDeNotas(dataArray, item, titulo) {
    ...
    // Adiciona matérias ao cabeçalho
    ...
    Object.keys(dataArray.alunos).forEach(function(cod) {
        ...
        for (var i = 0; i < dataArray.materias.length; i++) {
            ...
            // Tenta converter para numero se não for string especial
            if (!stringas.includes(Valor)) {
                let cc = parseFloat(Valor);
                if(!isNaN(cc)){
                    if(cc < 6) qtdeVermelha++;
                    row.push(cc); // Adiciona como número para o Excel
                } else {
                    row.push(Valor);
                }
            } else {
...
function MapaDeTodasNotas(dataArray) {
    ...
    // Configuração das linhas que serão geradas no Excel
    const linhasExcel = [
    ...
    Object.keys(dataArray.alunos).forEach(function(cod) {
        let faltaTotal = 0;
        let mediaano = new Array(dataArray.materias.length).fill(0); // Armazenará a SOMA das notas (com substituição)

        // Calcula as faltas totais e prepara média anual com regra de recomposição
        for (var matIdx = 0; matIdx < dataArray.materias.length; matIdx++) {
             ...
             if (materiaNotas) {
                  ...
                  // Lógica de Substituição de Notas (Recomposição)
        ...
        // Gera as linhas conforme a configuração
        linhasExcel.forEach(linhaCfg => {
            ...
            row.push(dataArray.alunos[cod].dadosAluno.nome);
            row.push(dataArray.alunos[cod].resultado);
            row.push(linhaCfg.label); // Nome do Período/Linha

            if (linhaCfg.tipo === 'nota') {
                for (var i = 0; i < dataArray.materias.length; i++) {
                    var Valor = '-';
                    if (dataArray.alunos[cod].notas[dataArray.materias[i]]) {
                        // Busca o valor pela chave (Ex: N1, NR1, N4...)
                ...
                // Linha de Média (Calculada com a soma das notas substituídas)
                ...
                // Linha Para Aprovação (Calculada com a soma das notas substituídas)
        ...
        // Linha em branco para separar alunos visualmente
        ws_data.push([]);
    });
...
function MapaParaAprovacao(dataArray) {
    ...
    Object.keys(dataArray.alunos).forEach(function(cod) {
        ...
        // Calcula somas com a lógica de substituição
        for (var i = 0; i < dataArray.materias.length; i++) {
             let materiaNotas = dataArray.alunos[cod].notas[dataArray.materias[i]];

             if (materiaNotas != undefined) {
                 // Recupera notas, tratando como 0 se não existir ou não for número
                 let n1 = parseFloat(materiaNotas['N1']) || 0;
                 let n2 = parseFloat(materiaNotas['N2']) || 0;
                 let n3 = parseFloat(materiaNotas['N3']) || 0;
                 let n4 = parseFloat(materiaNotas['N4']) || 0;
                 ...
                 // Lógica 1º Semestre (N1, N2 e NR1)
                 let finalN1 = n1;
                 let finalN2 = n2;
                 ...
                 // Lógica 2º Semestre (N3, N4 e NR2)
                 let finalN3 = n3;
                 let finalN4 = n4;
...
// ---------------- FIM DAS FUNÇÕES EXCEL ---------------- //

function semNotas(bim, dataArray) {
    let ws_data = [];
    // Cabeçalho para o arquivo Excel de notas faltantes
    ...
    Object.keys(dataArray.alunos).forEach(function(cod) {
        ...
        // Processa apenas alunos que estão "CURSANDO"
        if (aluno.resultado === "CURSANDO") {
            Object.keys(aluno.notas).forEach(function(materia) {
                // Itera de 1 até o bimestre selecionado
                for (let i = 1; i <= bim; i++) {
                    let notaKey = 'N' + i;
                    let valorNota = aluno.notas[materia][notaKey];

                    // Verifica se a nota é "-" (ausente) ou undefined
                    if (valorNota === "-" || valorNota === undefined) {
                        ws_data.push([
                            cod,
    ...
    // Se não houver dados (apenas o cabeçalho), avisa o usuário
    if (ws_data.length === 1) {
